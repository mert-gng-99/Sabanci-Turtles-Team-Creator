...
const translations = {
  tr: {
    ...
    team1: 'Takım 1 (Ev)',
    team2: 'Takım 2 (Dep)',
    selectAndClick: 'Önce oyuncuya, sonra takıma tıkla',
...
const getAutoPosition = (team, posArray) => {
  ...
  let x = 50, y = 50;
  // Jitter azaltıldı, mobil ekranlarda üst üste binmeyi azaltmak için
      ...
      <div className="w-full bg-slate-900/50 backdrop-blur-md border-b border-slate-700/50 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-14 md:h-16 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setLanguage('tr')} className={`px-2 py-1 rounded text-xs md:text-sm font-medium ${language==='tr'?'bg-indigo-600 text-white':'text-slate-400 hover:text-white'}`}>TR</button>
            <button onClick={() => setLanguage('en')} className={`px-2 py-1 rounded text-xs md:text-sm font-medium ${language==='en'?'bg-indigo-600 text-white':'text-slate-400 hover:text-white'}`}>EN</button>
          </div>
        </div>
      </div>

      {/* Main Container: Mobile için P-2, Desktop için P-4 */}
      <div className="max-w-7xl mx-auto p-2 md:p-4 mt-2 md:mt-6 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-8">
        
        {/* SOL PANEL: Oyuncu Havuzu */}
        <div className="order-2 lg:order-1 lg:col-span-3 space-y-4">
          <div className="bg-slate-800/60 backdrop-blur border border-slate-700 rounded-xl p-3 md:p-4 shadow-xl flex flex-col max-h-[400px] lg:h-auto lg:max-h-[calc(100vh-120px)] lg:sticky lg:top-24">
            ...
            <div className="flex-shrink-0 bg-slate-900/50 p-2 md:p-3 rounded-lg border border-slate-700/50 mb-3">
              ...
              <button 
                onClick={addPlayer}
                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold py-2 rounded transition shadow-lg shadow-indigo-500/20 active:scale-95"
              >
                EKLE
              </button>
            </div>
            ...
            <div className="flex-shrink-0 flex gap-1 mb-2 overflow-x-auto pb-1 custom-scrollbar">
              <button 
                ...
                className={`px-3 py-1 rounded-full text-[10px] md:text-xs whitespace-nowrap border transition
                  ${filterPos === 'ALL' 
              ...
              {getFilteredPlayers().map(player => {
                ...
                return (
                  <div 
                    ...
                    className={`group flex items-center justify-between p-2 rounded-lg cursor-pointer border transition-all duration-200 active:scale-[0.98]
                      ${isSelected 
                  ...
                  >
                    ...
                    <button 
                      ...
                      className="md:opacity-0 group-hover:opacity-100 text-slate-500 hover:text-rose-400 transition p-2"
                    >
                      <Icons.Trash size={16} />
                    </button>
                  </div>
        ...
        <div className="order-1 lg:order-2 lg:col-span-9 space-y-4 md:space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-6">
            {['team1', 'team2'].map((tm) => {
               ...
               const borderColor = tm === 'team1' ? 'border-red-500/20' : 'border-cyan-500/20';
               ...
               return (
                <div 
                  ...
                  className={`relative overflow-hidden rounded-xl border p-2 md:p-4 transition-all duration-300 cursor-pointer group active:bg-slate-700/60
                    ${selectedObj?.type === 'pool' 
                      ? 'border-indigo-500/50 bg-indigo-900/10 shadow-[0_0_15px_rgba(99,102,241,0.15)] animate-pulse' 
                      : 'border-slate-700 bg-slate-800/40'
                    }`}
                >
                  <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-2">
                    <h3 className={`font-bold ${colorClass} text-xs md:text-sm flex items-center gap-1`}>
                       <Icons.Shield size={14} /> <span className="truncate">{label}</span>
                    </h3>
                  ...
                  <div className="flex flex-wrap gap-1.5 min-h-[30px]">
                    {list.map((p, idx) => (
                      <div key={idx} className={`${bgBadge} ${borderColor} border ${tm==='team1'?'text-red-200':'text-cyan-200'} text-[10px] px-1.5 py-0.5 rounded flex items-center gap-1`}>
                    ...
                    {list.length === 0 && <span className="text-[10px] text-slate-500 italic opacity-50">...</span>}
                  </div>
                </div>
               )
            })}
          ...
          <div className="relative bg-slate-800 rounded-xl p-1 shadow-2xl border border-slate-700 overflow-hidden">
            {/* Mobilde saha biraz daha uzun (kareye yakın), masaüstünde geniş */}
            <div 
              ref={fieldRef}
              onClick={handleFieldClick}
              className={`relative w-full aspect-[1.6/1] md:aspect-[2.2/1] bg-gradient-to-br from-emerald-800 to-emerald-900 rounded-lg overflow-hidden border border-white/10 shadow-inner select-none touch-none
                ${selectedObj?.type === 'field' ? 'cursor-crosshair ring-2 ring-yellow-400/50' : ''}`}
            >
              <h3 className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full text-center text-white/10 font-bold text-3xl md:text-5xl uppercase tracking-widest pointer-events-none z-0">
                CHATV
              </h3>
              ...
              <div className="absolute top-1/2 left-3 md:left-5 w-8 h-20 md:w-12 md:h-32 border-2 border-l-0 border-white/30 transform -translate-y-1/2 pointer-events-none"></div>
              <div className="absolute top-1/2 right-3 md:right-5 w-8 h-20 md:w-12 md:h-32 border-2 border-r-0 border-white/30 transform -translate-y-1/2 pointer-events-none"></div>
              ...
              {[...team1, ...team2].map((player, idx) => {
                 ...
                 return (
                   <div 
                    ...
                    className={`absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all duration-200 flex flex-col items-center group z-10`}
                    style={{ left: `${player.x}%`, top: `${player.y}%`, touchAction: 'none' }}
                   >
                     {/* İkon Boyutu Mobilde w-7, Desktopta w-9 */}
                     <div className={`w-7 h-7 md:w-9 md:h-9 rounded-full shadow-lg border-2 flex items-center justify-center text-[9px] md:text-[10px] font-bold text-white relative transition-transform
                        ${isSel ? 'border-yellow-400 scale-125 ring-4 ring-yellow-400/30 z-50' : 'border-white'}
                        ${posColor}
                     `}>
                       {mainPos}
                       <div className={`absolute inset-0 rounded-full border-2 opacity-50 ${isTeam1 ? 'border-red-500' : 'border-cyan-500'}`}></div>
                     </div>
                     <span className="mt-1 text-[8px] md:text-[10px] font-bold text-white bg-black/60 px-1.5 py-0.5 rounded shadow backdrop-blur-sm whitespace-nowrap pointer-events-none">
                       {player.name}
                     </span>
                   </div>
                 );
            ...
            <div className="bg-slate-900/80 p-2 md:p-3 flex flex-col md:flex-row gap-2 md:justify-between items-center text-xs text-slate-400">
              ...
              <div className="flex w-full md:w-auto gap-2">
                 <button onClick={saveMatch} className="flex-1 md:flex-none justify-center flex items-center gap-1 hover:text-white bg-indigo-600/20 hover:bg-indigo-600 px-3 py-2 rounded transition border border-indigo-500/30 active:scale-95">
                   <Icons.Save size={14} /> {t.saveMatch}
                 </button>
                 <button onClick={handleDownload} className="flex-1 md:flex-none justify-center flex items-center gap-1 hover:text-white bg-slate-700/50 hover:bg-slate-600 px-3 py-2 rounded transition active:scale-95">
                   <Icons.Download size={14} /> {t.downloadPNG}
                 </button>
                 <button onClick={copyRosters} className="flex-1 md:flex-none justify-center flex items-center gap-1 hover:text-white bg-slate-700/50 hover:bg-slate-600 px-3 py-2 rounded transition active:scale-95">
                   <Icons.Copy size={14} /> {t.copyRosters}
                   {copyMessage && <span className="text-emerald-400 font-bold ml-1">✓</span>}
                 </button>
              </div>
            </div>
          </div>
          ...
          {/* --- GEÇMİŞ MAÇLAR --- */}
          <div className="order-3 space-y-4 pb-10">
            <h2 className="text-base md:text-xl font-bold flex items-center gap-2 text-indigo-300 border-b border-slate-700 pb-2">
              <Icons.History /> {t.matchHistory}
            </h2>

            {matches.length === 0 ? (
              <div className="text-center py-6 text-slate-500 border border-dashed border-slate-700 rounded-xl bg-slate-800/30 text-sm">
                {t.noMatches}
              </div>
            ) : (
              <div className="grid gap-3">
                {matches.map(match => (
                  <div key={match.id} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-lg relative group">
                    <button onClick={() => deleteMatch(match.id)} className="absolute top-2 right-2 text-slate-600 hover:text-rose-400 p-2">
                      <Icons.Trash size={14} />
                    </button>
                    ...
                    <div className="p-3 grid grid-cols-12 gap-1 items-center">
                      <div className="col-span-5 text-right">
                        ...
                        <div className="flex flex-wrap justify-end gap-1">
                          {match.t1.map((p, i) => (
                            <span key={i} className="text-[9px] bg-slate-700 px-1 py-0.5 rounded text-slate-300 truncate max-w-[50px]">{p.name}</span>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="col-span-2 flex flex-col items-center justify-center">
                        {editingScoreId === match.id ? (
                          ...
                          <div onClick={() => startEditingScore(match)} className="cursor-pointer bg-slate-900/50 px-2 py-1 rounded hover:bg-slate-700 transition text-center">
